# program written by Victor Silva for CMPUT 396 fall 2018
# algorithm based on Bruengger Marzetta Fukuda Nievergelt
#    parallel search bench ZRAM and its apps 
#    https://pdfs.semanticscholar.org/bb62/b77eee715f715a5e2347299f932b2fd3f29f.pdf
#    Ralph Udo Gasser
#    Harnessing Computational Resources for Efficient Exhaustive Search
#    phd thesis ETH Zurich 1995
...
class Board:
    ...
    def is_solved(self):
        return self.count_inversions() == 0 and self.empty == [self.w-1, self.h-1]
...
def rebuild_path(state):
    ...
    while state.parent != None:
        ...
        state = state.parent
    path.insert(0, state)
    print("Number of steps to solve: {}".format(len(path)))
    print('Solution')
    for i in path:
        print(i)
        print()
...
b = get_board_from_file('st44.txt')
print(b)
