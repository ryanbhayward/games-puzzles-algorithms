...
class AlphaBetaAgent(MinimaxAgent):
    ...
    def value(self, game_state, alpha, beta, time_available=-1, time_used=0,
        ...
        if game_state.is_terminal():
            ...
            debug.log({'MSG': 'Ahh!!'}, level=logging.INFO)            
            raise self.TimeIsUp
        else:
            tree['children'] = []
    ...
    def select_action(self, game_state, time_available=-1, max_depth=-1):
        """
        ...
        action found after `time_available` seconds. A non-positive
        `time_available` implies no time limit.
        ...
        for action in game_state.legal_actions():
            ...
            with game_state.play(action):
                try:
                    ...
                    log_time = time_available - (time.clock() - start_time)
