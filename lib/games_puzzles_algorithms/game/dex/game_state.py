...
class GameState(HexGameState):
    ...
    def __init__(self, acting_player, board, *groups):
    ...
    def potentially_winning_moves(self):
        if self._potentially_winning_moves is None:
            ...
            if self.num_actions_taken(self.acting_player) >= (self.board.size()[self.acting_player] - 1):
                original_group = self.win_detector.current_groups[self.acting_player].copy_raw()
                color = player_to_color(self.acting_player)
                for my_cell_index in self.board.my_cells[self.acting_player].keys():
                    for cell in self.board.every_legal_neighbor(
                        ...
                        if a not in self._potentially_winning_moves:
                            ...
                            if self.win_detector.check_if_winner(self.acting_player):
                                self._potentially_winning_moves[a] = True
                            self.win_detector.current_groups[self.acting_player].copy_from_raw(original_group)
    ...
    def every_empty_cell(self):
        for row, column in self.board.every_empty_cell(self.acting_player):
            yield row, column
    ...
    def num_undominated_empty_cells(self):
        ...
        return (
            ...
            self.board.num_legal_actions(self.acting_player)
        )
